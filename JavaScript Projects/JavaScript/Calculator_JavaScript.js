...
const Calculator = {
    ...
    Display_Value: '0',
    //this will hold the first operand for any expressions, it is set to null for now
    First_Operand: null,
...
function Input_Digit (digit) {
    ...
    //this is checking to see if Wait_Second_Operand is true and set Display_Value to the key that was clicked
...
//this is saying that if the Display_Value does not contain a decimal point we want to add a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed it converts the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist
...
keys.addEventListener('click', (event) => {
    //the target variable is an object that reprents the element that was clicked
    const { target } = event;
